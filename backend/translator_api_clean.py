#!/usr/bin/env python3
"""
PDF Translation API v8.8 - Clean Version
Translates to English and filters out OpenAI error messages
"""
...
app = FastAPI(title="PDF Translation API v8.8 - Clean English Edition")
...
def extract_text_blocks(pdf_path: str) -> List[Dict]:
    """Extract text blocks with positioning from PDF"""
...
def translate_text_clean(text: str, target_language: str = "English") -> str:
    """Translate text to English using OpenAI with error filtering"""
    
    # Skip translation for very short or non-meaningful text
    if len(text.strip()) < 3 or text.strip() in ['~', '.', '..', '...', 'lt', ';;;', 'ê', '"', "'"]:
        return ""
    ...
    messages = [
        {
            "role": "system", 
            "content": f"You are a professional translator. Translate the given text to {target_language}. Maintain proper formatting, capitalization, and professional tone. Return only the translation without any explanations or apologies. If the text is unclear or incomplete, return an empty string."
        },
        {
            "role": "user", 
            "content": text
...
def create_translated_pdf_clean(text_blocks: List[Dict], visual_elements: List[Dict], target_language: str = "English") -> str:
    """Create translated PDF with error filtering"""
    ...
    for page_num in range(max_page + 1):
        ...
        for block in page_data['text']:
            try:
                ...
                translated_text = translate_text_clean(original_text, "English")
                
                # Skip rendering if translation is empty (error message filtered out)
                if not translated_text:
                    print(f"Block {len([b for b in page_data['text'] if page_data['text'].index(b) <= page_data['text'].index(block)])}: Skipped (empty translation)")
                    continue
...
async def startup_event():
    ...
    print("🚀 Initializing PDF Translation API v8.8 - Clean English Edition")
...
async def translate_pdf_endpoint(file: UploadFile = File(...), target_language: str = "English"):
    """Translate PDF to English with error filtering"""
    ...
    try:
        ...
        # Create translated PDF with error filtering
        temp_output = create_translated_pdf_clean(text_blocks, visual_elements, target_language)
        ...
        return FileResponse(
            ...
            filename=f"clean_translated_{file.filename}"
        )
...
async def health_check():
    ...
    return {"status": "healthy", "version": "8.8-clean"}
